...
export const useFocus = () => {
  ...
  useEffect(() => {
    ...
    setFocusTime(fTime);
    setShortBreak(sBreak);
    setLongBreak(lBreak);

    // 根据当前阶段设置时间
    let newTimeMinutes = fTime;
    if (currentPhase === "shortBreak") newTimeMinutes = sBreak;
    else if (currentPhase === "longBreak") newTimeMinutes = lBreak;

    const newTimeLeft = newTimeMinutes * 60;
  ...
  // 计算下一个阶段
  const calculateNextPhase = useCallback(
    (currentPhase: TimerPhase, currentCount: number): TimerPhase => {
  ...
  const switchToNextPhase = useCallback(() => {
    ...
    if (
      (nextPhase === "shortBreak" || nextPhase === "longBreak") &&
      focusSettingsValues.autoStartBreak
    ) {
      startBreakTimer();
    }
  }, [
